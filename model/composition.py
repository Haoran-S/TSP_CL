import torch
from .common import MLP, weighted_mse_loss
from .common import weighted_ratio_loss
import time
...
class Net(torch.nn.Module):
    ...
    def observe(self, x, t, y, loss_type="MSE", x_te=None, x_tr=None, scale=1.0):
            ...
            for i in range(0, x.size()[0], self.mini_batch_size):
                ...
                g_loss = torch.sum(
                    torch.exp(
                        scale
                        * self.loss_dual(
                            batch_x1, self.forward(batch_x1, t), batch_y1, self.noise
                        )
                    )
                )
                ...
                temp_w = torch.exp(
                    scale
                    * self.loss_dual(
                        batch_x2, self.forward(batch_x2, t), batch_y2, self.noise
                    )
                )
                temp_numerator = self.loss_wmse(
                    self.forward(batch_x2, t), batch_y2, temp_w
                )
                ...
                with torch.no_grad():
                    grads_squared_new = []
                    for param, g_g, f2_g, square_g in zip(
                        self.net.parameters(), g_grad, f2_grad, grads_squared
                    ):
        ...
        set_w = torch.exp(
            scale * self.loss_dual(set_x, self.forward(set_x, t), set_y, self.noise)
        )
        ...
        indices_combine = torch.cat(
            (
                indices_first[0 : round(self.memories * (1.0 - 1.0 / (self.age + 1)))],
